          ...
          <table class="table table-striped projects">
            ...
            <tbody>
              <tr v-for="(student, index) in students" :key="index">
                ...
                <td>
                  <input
                    v-if="student.present === true"
                    type="checkbox"
                    checked
                    disabled
                  />
                  <input
                    v-else-if="student.present === false"
                    type="checkbox"
                    disabled
                  />
...
export default {
  ...
  asyncData({ params, redirect, $axios }) {
    ...
    return $axios
      .post("/api/studentattendance/getStudentsAttendanceBySubjectID", data)
      .then(res => {
        return $axios
          .post("/api/studentattendance/allowed", data2)
          .then(res2 => {
  ...
  methods: {
    dateChange() {
      ...
      this.$axios
        .post("/api/studentattendance/getStudentsAttendanceBySubjectID", data2)
        .then(res2 => {
          this.$axios
            .post("/api/studentattendance/allowed", data3)
            .then(res3 => {
    ...
    addAttendance(present, studentid, subjectid) {
      ...
      this.$axios.post("/api/studentattendance/new", data).then(res => {
        ...
        this.$axios
          .post(
            "/api/studentattendance/getStudentsAttendanceBySubjectID",
            data2
          )
          .then(res2 => {
            this.$axios
